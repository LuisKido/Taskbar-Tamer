...
public class PartFactoryTests
{
    ...
    {
        Part comun = PartFactory.Create(1, "Abisal", AnatomySlot.Claws, Rarity.Comun, Config);
        Part epico = PartFactory.Create(2, "Abisal", AnatomySlot.Claws, Rarity.Epico, Config);
    ...
    {
        Part claws = PartFactory.Create(1, "Abisal", AnatomySlot.Claws, Rarity.Comun, Config);
    ...
    {
        Part shell = PartFactory.Create(1, "Abisal", AnatomySlot.Shell, Rarity.Comun, Config);
        Assert.True(shell.BaseStats.MaxHp > 0);
        Assert.Equal(0, shell.BaseStats.Attack);
    }

    [Fact]
    public void Comun_aplica_factor_x1()
    {
        Part comun = PartFactory.Create(1, "Abisal", AnatomySlot.Claws, Rarity.Comun, Config);
        // La plantilla base de Claws es Attack 50 y Comun = x1.0.
